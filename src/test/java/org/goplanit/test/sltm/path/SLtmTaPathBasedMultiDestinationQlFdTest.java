 ...
 * using a quadratic linear shaped Fundamental diagram
 * 
 * @author markr
 *
 */
public class SLtmTaPathBasedMultiDestinationQlFdTest extends sLtmAssignmentMultiDestinationTestBase {

  /** the logger */
  private static Logger LOGGER = null;

  private void testStochasticLinkOutputs(StaticLtm sLTM) {
  ...
  public static void setUp() throws Exception {
    if (LOGGER == null) {
      LOGGER = Logging.createLogger(SLtmTaPathBasedMultiDestinationQlFdTest.class);
    }
  }
  ...
  /**
   * Test sLTM path based assignment on above network for a point queue model with MNL SUE
   */
  @Test
  public void sLtmPointQueuePathBasedMnlSueAssignmentTest() {
    try {
      ...
      configurator.createAndRegisterFundamentalDiagram(FundamentalDiagram.QUADRATIC_LINEAR); // <-- this is the difference
      ...
      //var smoothing = (FixedStepSmoothingConfigurator) configurator.createAndRegisterSmoothing(Smoothing.FIXED_STEP);
      //smoothing.setStepSize(1);

      /* MSRA - aggressive due to no overlap with other ODs*/
      var smoothing = (MSRASmoothingConfigurator) configurator.createAndRegisterSmoothing(Smoothing.MSRA);
      smoothing.setActivateLambda(true);
      smoothing.setKappaStep(1);
      smoothing.setGammaStep(-0.05);

      configurator.addTrackOdsForLogging(IdMapperType.ID, Pair.of(0,2));
      ...
      {
        /* MNL for path choice */
        var choiceModel = suePathChoice.createAndRegisterChoiceModel(ChoiceModel.MNL);
        choiceModel.setScalingFactor(7/0.2); // normalise directly as min cost ~0.2h

        suePathChoice.getPathFilter().activateMaxOverlapFilter(0.6);
      }
      ...
      testStochasticLinkOutputs(sLTM);
