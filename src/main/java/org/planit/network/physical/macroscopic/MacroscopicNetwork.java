...
public class MacroscopicNetwork extends PhysicalNetwork {
  ...
  /**
   * Map containing the BPR parameters for link segment and mode, if these are specified in the
   * network file (null if
   ...
   */
  public MacroscopicLinkSegmentType createAndRegisterNewMacroscopicLinkSegmentType(final String name,
      final double capacity, final double maximumDensity,
      final Object linkSegmentExternalId, final Map<Mode, MacroscopicModeProperties> modeProperties)
    ...
    MacroscopicLinkSegmentType linkSegmentType = ((MacroscopicNetworkBuilder) networkBuilder).createLinkSegmentType(
        name, capacity, maximumDensity, linkSegmentExternalId,
        modeProperties);
   ...
   * 
   * This method has the option to convert the external Id parameter into a long value, to find the
   * link segment type when
   ...
   * Retrieve a link segment type by its external Id
   * 
   * This method is not efficient, since it loops through all the registered modes in order to find
   * the required link
   * segment type. The equivalent method in InputBuilderListener is more efficient and should be
