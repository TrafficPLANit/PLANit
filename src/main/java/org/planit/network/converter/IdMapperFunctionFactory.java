...
/**
 * Factory that creates functions for id mapping from PLANit ids to ids to be used for persistence. Based on the passed in IdMapper type
 * functions will generate different ids when applied to nodes, link segments, etc.
 * 
 * @author markr
 *
 */
public class IdMapperFunctionFactory {
   ...
   * 
   */
  protected static <T extends ExternalIdable> BiFunction<T, IdGroupingToken, String> createIdMappingFunction(Class<T> Clazz, final IdMapperType idMapper) throws PlanItException {        
    switch (idMapper) {
    case ID:
      return (instance, theIdToken) -> { return Long.toString(instance.getId());};
    case EXTERNAL_ID:
      return (instance, theIdToken) -> { return String.format("%s",instance.getExternalId());};      
    case GENERATED:
      return (instance, theIdToken) -> { return Long.toString(IdGenerator.generateId(theIdToken, Clazz));};
    default:
   ...
   * @throws PlanItException thrown if error
   */
  public static BiFunction<Node, IdGroupingToken, String> createNodeIdMappingFunction(final IdMapperType idMapper) throws PlanItException {
   ...
   * @throws PlanItException thrown if error
   */
  public static BiFunction<Link, IdGroupingToken, String> createLinkIdMappingFunction(final IdMapperType idMapper) throws PlanItException {        
   ...
   * @throws PlanItException thrown if error
   */  
  public static BiFunction<MacroscopicLinkSegmentType, IdGroupingToken, String> createLinkSegmentTypeIdMappingFunction(
      IdMapperType idMapper) throws PlanItException {
    return createIdMappingFunction(MacroscopicLinkSegmentType.class,idMapper);
  }  
  
  /** create a function that takes a link segment and (optional) ide mapper and generates the appropriate MATSIM link id based on the user configuration
   * id grouping token is only used when mapper is set to GENERATE, in which case the token is used for generating ids
   * 
   * @return function that generates mapped link segment id's for persistence
   * @throws PlanItException thrown if error
   */
  public static BiFunction<MacroscopicLinkSegment, IdGroupingToken, String> createLinkSegmentIdMappingFunction(final IdMapperType idMapper) throws PlanItException {
    switch (idMapper) {
    case ID:
      return (macroscopicLinkSegment, theIdToken) -> { return Long.toString(macroscopicLinkSegment.getId());};
    case EXTERNAL_ID:
      return (macroscopicLinkSegment, theIdToken) -> {
    ...
    case GENERATED:
      return (macroscopicLinkSegment, theIdToken) -> { return Long.toString(IdGenerator.generateId(theIdToken, MacroscopicLinkSegment.class));};
    default:
