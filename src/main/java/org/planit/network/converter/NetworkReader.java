...
public interface NetworkReader {
  ...
  /**
   * Each reader maps the modes of the original reader format to PLANit modes. This mapping should be made available via this method
   * It is required for any network converter to verify if the planit modes that are used are compatible with the planit modes that the writer
   * expects and in turn maps onto the writer's output format modes 
   *   
