...
public abstract class ODOutputTypeAdapterImpl extends OutputTypeAdapterImpl implements ODOutputTypeAdapter {
        ...
        default:
          return new PlanItException(
              "Tried to find link property of " + BaseOutputProperty.convertToBaseOutputProperty(outputProperty)
                  .getName() + " which is not applicable for OD matrix.");
      }
  ...
  /**
   * ODSkimOutputType.COST: Cost is collected through the shortest path in iteration i based on the
   * link costs of
   * iteration i-1, so the od cost of i-1 are only known once we are in iteration i, hence this
   * information is trailing
   * behind one iteration, and we can only store it in i. Hence, we must reduce the iteration index
   * by 1 to obtain the
