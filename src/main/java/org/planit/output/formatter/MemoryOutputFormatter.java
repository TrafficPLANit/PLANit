...
/**
 * OutputFormatter which stores data in memory, using specified keys and output
 * properties.
 * 
 * @author gman6028
 *
...
public class MemoryOutputFormatter extends BaseOutputFormatter {
	 ...
	 * @param outputProperties        OutputProperty array of result types to be
	 *                                recorded
	 * @param outputKeys              OutputProperty array of key types to be
	 *                                recorded
	 * @param linkSegment             the current link segment
	 * @param linkOutputTypeAdapter   output adapter to provide methods to get the
	 *                                property values
	 * @param mode                    the current mode
	 * @param timePeriod              the current time period
	 * @param recordLinksWithZeroFlow true if links with zero flow are to be
	 *                                recorded
	 * @throws PlanItException thrown if there is an error
	 */
	private void updateOutputAndKeyValuesForLink(MultiKeyPlanItData multiKeyPlanItData,
			...
			boolean recordLinksWithZeroFlow) throws PlanItException {
		if (recordLinksWithZeroFlow || linkOutputTypeAdapter.isFlowPositive(linkSegment, mode)) {
			updateOutputAndKeyValues(multiKeyPlanItData, outputProperties, outputKeys, (label) -> {
				return linkOutputTypeAdapter.getLinkOutputPropertyValue(label, linkSegment, mode, timePeriod,
	 ...
	 * @param odOutputTypeAdapter output adapter to provide methods to get the
	 *                            property values
	 * @param mode                the current mode
	 * @param timePeriod          the current time period
	 * @throws PlanItException thrown if there is an error
	...
	private void updateOutputAndKeyValuesForOD(MultiKeyPlanItData multiKeyPlanItData, OutputProperty[] outputProperties,
			OutputProperty[] outputKeys, ODMatrixIterator odMatrixIterator, ODOutputTypeAdapter odOutputTypeAdapter,
			Mode mode, TimePeriod timePeriod) throws PlanItException {
		odMatrixIterator.next();
		updateOutputAndKeyValues(multiKeyPlanItData, outputProperties, outputKeys, (label) -> {
			return odOutputTypeAdapter.getODOutputPropertyValue(label, odMatrixIterator, mode, timePeriod,
					outputTimeUnit.getMultiplier());
		});
	}
	 ...
	 *                              recorded
	 * @param outputKeys            OutputProperty array of key types to be recorded
	 * @param odPathIterator        ODPathIterator to iterate through matrix of
	 *                              paths
	 * @param pathOutputTypeAdapter PathOutputTypeAdapter to provide methods to get
	 *                              the property values
	 * @param mode                  the current mode
	 * @param timePeriod            the current time period
	...
	private void updateOutputAndKeyValuesForPath(MultiKeyPlanItData multiKeyPlanItData,
			OutputProperty[] outputProperties, OutputProperty[] outputKeys, ODRouteIterator odPathIterator,
			RouteOutputTypeAdapter pathOutputTypeAdapter, Mode mode, TimePeriod timePeriod, RouteIdType pathIdType)
			throws PlanItException {
		odPathIterator.next();
		updateOutputAndKeyValues(multiKeyPlanItData, outputProperties, outputKeys, (label) -> {
			return pathOutputTypeAdapter.getRouteOutputPropertyValue(label, odPathIterator, mode, timePeriod,
					pathIdType);
		});
	}
	 ...
	 *                                persistence
	 * @param currentOutputType,      the active output type of the configuration we
	 *                                are persisting for (can be a suboutputtype)
	...
	protected void writeLinkResultsForCurrentTimePeriod(OutputTypeConfiguration outputTypeConfiguration,
			OutputTypeEnum currentOutputType, OutputAdapter outputAdapter, Set<Mode> modes, TimePeriod timePeriod,
			int iterationIndex) throws PlanItException {
		// for links we assume no sub-output types exist (yet), hence this check to make
		// sure we can
		// cast safely
		...
		LinkOutputTypeAdapter linkOutputTypeAdapter = (LinkOutputTypeAdapter) outputAdapter
	...
	protected void writePathResultsForCurrentTimePeriod(OutputTypeConfiguration outputTypeConfiguration,
		...
		OutputProperty[] outputProperties = outputValueProperties.get(outputType);
		OutputProperty[] outputKeys = outputKeyProperties.get(outputType);
		RouteOutputTypeAdapter pathOutputTypeAdapter = (RouteOutputTypeAdapter) outputAdapter
				.getOutputTypeAdapter(outputType);
		...
		for (Mode mode : modes) {
			...
			for (ODRouteIterator odPathIterator = odPathMatrix.iterator(); odPathIterator.hasNext();) {
				updateOutputAndKeyValuesForPath(multiKeyPlanItData, outputProperties, outputKeys, odPathIterator,
						pathOutputTypeAdapter, mode, timePeriod, pathOutputTypeConfiguration.getPathIdType());
			}
			timeModeOutputTypeIterationDataMap.put(mode, timePeriod, iterationIndex, outputType, multiKeyPlanItData);
		}
	}
	...
	public Object getOutputDataValue(Mode mode, TimePeriod timePeriod, Integer iterationIndex, OutputType outputType,
		...
		MultiKeyPlanItData multiKeyPlanItData = (MultiKeyPlanItData) timeModeOutputTypeIterationDataMap.get(mode,
				timePeriod, iterationIndex, outputType);
	 ...
	 * 
	 * @param outputTypeConfigurations OutputTypeConfigurations for the assignment
	...
	public int getLastIteration() {
		Set<MultiKey<? extends Object>> keySet = (Set<MultiKey<? extends Object>>) timeModeOutputTypeIterationDataMap
				.keySet();
		int lastIteration = 0;
	...
	/**
	 * Returns a MemoryOutputIterator for the contents of the specified
	 * MultiKeyPlanItData map
	 * 
	 * @param mode           value of mode key
	 * @param timePeriod     value of time period key
	 * @param iterationIndex value of iteration index key
	 ...
	 */
	public MemoryOutputIterator getIterator(final Mode mode, final TimePeriod timePeriod, final Integer iterationIndex,
	 ...
	 * @param iterationIndex      value of iteration index key
	 * @param outputType          value of output type key
	 * @param outputValueProperty the output value property whose position is
	 *                            required
	 ...
	 */
	public int getPositionOfOutputValueProperty(final Mode mode, final TimePeriod timePeriod,
			final Integer iterationIndex, final OutputType outputType, final OutputProperty outputValueProperty)
			throws PlanItException {
		MultiKeyPlanItData multiKeyPlanItData = (MultiKeyPlanItData) timeModeOutputTypeIterationDataMap.get(mode,
				timePeriod, iterationIndex, outputType);
	 ...
	 */
	public int getPositionOfOutputKeyProperty(final Mode mode, final TimePeriod timePeriod,
			final Integer iterationIndex, final OutputType outputType, final OutputProperty outputKeyProperty)
			throws PlanItException {
		MultiKeyPlanItData multiKeyPlanItData = (MultiKeyPlanItData) timeModeOutputTypeIterationDataMap.get(mode,
				timePeriod, iterationIndex, outputType);
