...
public class ZoningModifierImpl implements ZoningModifier {
  ...
  public void recreateZoneIds() {
    zoningBuilder.recreateZoneIds(zoning.odZones, zoning.transferZones);
  }  
  ...
  public void removeDanglingTransferZoneGroups() {
    ...
    for(TransferZoneGroup group : zoning.transferZoneGroups) {
      if(group.isEmpty()) {
        zoning.transferZoneGroups.remove(group);
        groupRemoved = true;
      }
    }
