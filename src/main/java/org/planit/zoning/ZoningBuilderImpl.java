...
public class ZoningBuilderImpl implements ZoningBuilder{
  ...
  /** recreate ids for the connectoids container provided where we use the id generator based on the theClass token, while we set the generated id using the method
   * provided via the biConsumer
   * 
    ...
    if(connectoids.size()>0) {      
      for (Connectoid connectoid : connectoids) {
        if (theClass.isInstance(connectoid)) {
          ...
          LOGGER.severe(String.format("attempting to reset id on connectoid (%s) that is not compatible with the connectoid implementation generated by this builder, ignored", connectoid.getClass().getCanonicalName()));
        }
      }            
    }      
  }
  
  /** recreate ids for the zones container provided where we use the id generator based on the theClass token, while we set the generated id using the method
   * provided via the biConsumer
   * 
    ...
    if(zones.size()>0) {      
      for (Zone zone : zones) {
        if (theClass.isInstance(zone)) {
          ...
          LOGGER.severe(String.format("attempting to reset id on connectoid (%s) that is not compatible with the connectoid implementation generated by this builder, ignored", zone.getClass().getCanonicalName()));
        }
      }            
    }      
  }  
  ...
  private <T extends Connectoid> void recreateConnectoidIdIndexMapping(Connectoids<T> connectoids) {
    if(connectoids instanceof ConnectoidsImpl<?>) {
      ...
      LOGGER.severe(String.format("attempting to update id mapping on connectoids (%s) that is not compatible with the connectoids implementation generated by this builder, ignored", connectoids.getClass().getCanonicalName()));      
    } 
  }   
  ...
  private <T extends Zone> void recreateZoneIdIndexMapping(Zones<T> zones) {
    if(zones instanceof ZonesImpl<?>) {
      ...
      LOGGER.severe(String.format("attempting to update id mapping on zones (%s) that is not compatible with the zones implementation generated by this builder, ignored", zones.getClass().getCanonicalName()));      
    } 
  }     
  ...
  protected <T extends Connectoid> void recreateConnectoidIds(Connectoids<T> connectoids) {
    BiConsumer<ConnectoidImpl, Long> biConsumer = (connectoid, newId) -> {connectoid.setId(newId);};
  ...
  protected <T extends Zone> void recreateZoneIds(Zones<T> zones) {
    BiConsumer<ZoneImpl, Long> biConsumer = (zone, newId) -> {zone.setId(newId);};
  ...
  protected void recreateUndirectedConnectoidIds(UndirectedConnectoids undirectedConnectoids) {
    BiConsumer<UndirectedConnectoidImpl, Long> biComsumer = (connectoid, newId) -> {connectoid.setUndirectedConnectoidId(newId);};
  ...
  */  
  protected  void recreateDirectedConnectoidIds(DirectedConnectoids directedConnectoids) {
    BiConsumer<DirectedConnectoidImpl, Long> biComsumer = (connectoid, newId) -> {connectoid.setDirectedConnectoidId(newId);};
  ...
  */  
  protected void recreateOdZoneIds(Zones<OdZone> odZones) {
    BiConsumer<OdZoneImpl, Long> biComsumer = (odZone, newId) -> {odZone.setOdZoneId(newId);};
  ...
  */  
  protected void recreateTransferZoneIds(Zones<TransferZone> transferZones) {
    BiConsumer<TransferZoneImpl, Long> biComsumer = (odZone, newId) -> {odZone.setTransferZoneId(newId);};
   ...
   */  
  @Override
  public void recreateZoneIds(Zones<OdZone> odZones, Zones<TransferZone> transferZones) {
    /* first reset the zone class ids using the base class signature and token */
    if(!odZones.isEmpty() || !transferZones.isEmpty()) {
  ...
  public void recreateTransferZoneGroupIds(TransferZoneGroups transferZoneGroups) {    
    if(!transferZoneGroups.isEmpty()) {      
      ...
      for (TransferZoneGroup group : transferZoneGroups) {
        if (TransferZoneGroupImpl.class.isInstance(group)) {
          ...
          LOGGER.severe(String.format("attempting to reset id on transfer zone group (%s) that is not compatible with the transfer zone group implementation generated by this builder, ignored", group.getClass().getCanonicalName()));
        }
      }            
    } 
    ...
    if(transferZoneGroups instanceof TransferZoneGroupsImpl) {
      ...
      LOGGER.severe(String.format("attempting to update id mapping on transfer zone groups (%s) that is not compatible with the transfer zone groups implementation generated by this builder, ignored", transferZoneGroups.getClass().getCanonicalName()));      
    } 
  }
