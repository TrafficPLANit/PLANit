...
/**
 * Inner class that holds the mapping of the inputs to/from the underlying physical network (if any). Currently we support
 * the PLANit network format for this mapping, or one provides the fixed input separate from a particular network format 
 * 
 * By default we:
...
public class TampereNodeModelFixedInput{
  ...
  /** Constructor. The TampereNodeModelFixedInput class is meant to be created once for each node where the node model is applied
