...
public class RoutedServicesLayerModifierImpl extends EventProducerImpl implements RoutedServicesLayerModifier {
  ...
  /**
   * Recursive method supporting both #RoutedTripFrequency and #RoutedTripSchedule truncations. Based on the given
   * offset related to the to be removed timing segments, identifies the first upcoming valid chain after this offset.
   * If found, it creates a copy of the original and removes all segments around the identified valid chain and adds
   * it to the list of new "truncated" routed trip schedules (or frequecnies) to replace the original after finishing.
   * It is assumed at least part of the schedule or frequency's segments are valid (is to be checked beforehand)
   *
   ...
   * @return  list of newly created truncated trip schedules or frequencies (not yet registered on their
   *          routed services container), if any and maybe null
   */
  private <T extends RoutedTrip> List<T> recursiveTruncateXTripChainToServiceNetwork(
    ...
    if(!isTripSchedule && !(freqOrSched instanceof RoutedTripFrequency)){
      throw new PlanItRunTimeException("Only routed trip schedule or routed trip frequencies " +
          "are supported in recursive truncation to service network");
    }
    ...
    // check for special case where nothing is to be removed for the very first chain, i.e., first removal
    // index is after first chain final index
    ...
    if(!initialChainWithoutRemovalBeforeChain &&
        ListUtils.getLastValue(consecutiveLegsToRemoveBeforeChain) == (numSegments-1)){
    ...
    if(!lastChainWithoutRemovalAfter){
      ...
      final var allIndicesAfterLastChainLeg =
          IntegerListUtils.rangeOf(firstLegIndexValueToRemoveAfterChain,numSegments);
  ...
  /**
   * Truncate to service network, see {@link #recursiveTruncateXTripChainToServiceNetwork(
   * int, List, RoutedTrip, RoutedTripFactory)} for details
   *
  ...
  private List<RoutedTripFrequency> truncateFrequencyTripChainToServiceNetwork(
          List<Integer> toBeRemovedLegSegments,
          RoutedTripFrequency routedTripFrequency,
          RoutedTripFrequencyFactory factory) {
  ...
  /**
   * Truncate to service network, see {@link #recursiveTruncateXTripChainToServiceNetwork(int,
   * List, RoutedTrip, RoutedTripFactory)} for details
   *
  ...
  private List<RoutedTripSchedule> truncateScheduledTripChainToServiceNetwork(
          final List<Integer> toBeRemovedRelTimingSegments,
          final RoutedTripSchedule routedTripSchedule,
          RoutedTripScheduleFactory factory) {
    /* bootstrap recursive call */
    return recursiveTruncateXTripChainToServiceNetwork(
        -1, toBeRemovedRelTimingSegments, routedTripSchedule, factory);
  }

  /**
   * adjust the passed in instance to the validly mapped portion of it. This means, no changes, or full removal of all
   * leg segments and reset to 0 frequency, or partial removal, meaning splitting the original entry into partially
   * copied entries (and resetting the original)
   *
   * @param routedTripFrequency a routed trip(s) group for a given schedule to truncate to underlying service network
   * @param factory to use when we need to split a routedTripSchedule in multiple entries due to possible gaps that
   *                are identified
   * @return new partial frequencies (if any)
   */
  private List<RoutedTripFrequency> truncateFrequencyTripToServiceNetwork(
      final RoutedTripFrequency routedTripFrequency, RoutedTripFrequencyFactory factory) {

    /* identify the no longer valid leg segments */
    var toBeRemovedLegSegments =
            RoutedTripFrequencyUtils.findServiceLegSegmentsNotMappedToServiceNetwork(
                routedTripFrequency, this.routedServicesLayer.getParentLayer());
    ...
    // Replace existing routed trip schedule by one or more truncated ones, one per identified consecutive
    // chain with adjusted departure times
    ...
    if(truncatedRoutedTripFrequencies == null || truncatedRoutedTripFrequencies.isEmpty()){
      throw new PlanItRunTimeException("Invalid truncation of routed trip frequency, expected at least one alternative " +
          "to be created");
    }
    // now mark existing schedule for removal, it is replaced by partial (truncated) copies reflecting new
    // valid sub-trips
    routedTripFrequency.clear();
  ...
  /**
   * Truncate routed trip(s) - schedule based -  at hand to the remaining service network (which likely has undergone
   * changes). This means removing parts of a trip schedule that is no longer present or valid, or even splitting
   * into multiple trip schedules in case gaps within the same entry are detected
   * <p>
   * Ids are not recreated in this call, only truncation is performed
   * </p>
   * <p>
   * It is possible the routed trip schedule ends up without any departures or leg timings in which case the
   * invoker is expected to cull the schedule as it is no longer viable.
   * </p>
   *
   ...
   *                are identified
   */
  private List<RoutedTripSchedule> truncateScheduledTripToServiceNetwork(
      final RoutedTripSchedule routedTripSchedule, RoutedTripScheduleFactory factory) {

    /* identify the rel timing leg segments */
    List<Integer> toBeRemovedRelativeTimingLegSegments =
        RelativeLegTimingUtils.findLegTimingsNotMappedToServiceNetwork(routedTripSchedule, this.routedServicesLayer);
    ...
    /* (Recursive) replace existing routed trip schedule by one or more truncated ones, one per identified
    consecutive chain with adjusted departure times */
    List<RoutedTripSchedule> truncatedRoutedTripSchedules =
            truncateScheduledTripChainToServiceNetwork(
                toBeRemovedRelativeTimingLegSegments, routedTripSchedule, factory);
    if(truncatedRoutedTripSchedules == null || truncatedRoutedTripSchedules.isEmpty()){
      throw new PlanItRunTimeException("Invalid truncation of routed trip schedule, expected at " +
          "least one alternative schedule to be created");
    }
    /* now mark existing schedule for removal, it is replaced by partial (truncated) copies
    reflecting new valid sub-trips */
    routedTripSchedule.clear();
  ...
  /**
   * Truncate routed service's frequency based trips to the remaining service network (which likely
   ...
   * in the result, the original has been cleared and removed from the service container,
   * it only acts as a placeholder key. Created truncated frequencies have NOT yet been registered on the container
   */
  private Map<RoutedTripFrequency, Collection<RoutedTripFrequency>> truncateRoutedServiceFrequenciesToServiceNetwork(
   ...
   * @return replacement schedules created (if any) for each schedule of the routed service. When replacement schedules
   * exist in the result, the original schedule has been cleared and removed from the service container, it only acts
   * as a placeholder. Created truncated schedules have NOT yet been registered on the container
   *
   */
  private Map<RoutedTripSchedule, Collection<RoutedTripSchedule>> createTruncatedRoutedServiceSchedulesToServiceNetwork(
      RoutedService routedService) {
  ...
  /**
   * Truncate all frequency based routed services for a mode and match them to the remaining service network (which
   * likely has undergone changes), i.e., remove all service network entities that are now missing. If, for
   * some reason, the provided services by mode have no (more) entries the services by mode are removed from the layer.
   * <p>
   *   Ids are not recreated in this call, only truncation is performed and truncated schedules are registered
   *   internally on each service
   * </p>
   *
  ...
  /**
   * Truncate all schedule based routed services for a mode and match them to the remaining service network
   * (which likely has undergone changes), i.e., remove all service network entities that are now missing.
   * If, for some reason, the provided services by mode have no (more) entries the services by mode are
  ...
  /**
   * Truncate all routed services for a mode and match them to the remaining service network (which likely has
   * undergone changes), i.e., remove/truncate all routed services (and their trips) relying on service network
   * entities that are now missing.
   * <p>
   *   Ids are not recreated in this call, only truncation is performed and truncated schedules are registered
   *   internally on each service
   * </p>
  ...
  protected void fireEvent(EventListener eventListener, Event event) {
    ((RoutedServicesModifierListener) eventListener).onRoutedServicesModifierEvent(
        (RoutedServicesModificationEvent) event);
  }
   ...
   * {@inheritDoc}
   *
   *  Note that this implementation will automatically overwrite all pre-existing XML ids with the internal
   *  ids of all managed id containers within the routed services layer to ensure  uniqueness on both levels of ids.
   *  <p>
   *    Listeners can be registered for existing id modification events
   *  </p>
   *
   *  If, for some reason, the provided services by mode have no (more) entries the services by mode are removed
  ...
  public void truncateToServiceNetwork(){
    ...
    /* after truncation routed services all internal ids need to be recreated to ensure contiguous ids
    throughout the routed services */
  ...
  public void consolidateIdenticallyScheduledTrips(Mode mode) {
    ...
    for(var routedService : servicesByMode){
      ...
      var groupedByRelativeSchedule = scheduleBasedTrips.groupBy(
          rts -> rts.getRelativeLegTimingsAsStream().collect(Collectors.toList()));
      for(var schedulesToConsolidate : groupedByRelativeSchedule.values()){
        ...
        schedulesToConsolidate.stream().flatMap(
            schedule -> schedule.getDepartures().stream()).forEach(
                referenceDeparturesToSupplement::register);
        ...
        schedulesToConsolidate.stream().filter(ExternalIdAble::hasExternalId).forEach(
            s -> referenceTripSchedule.appendExternalId(s.getExternalId(), CharacterUtils.COMMA));
  ...
  public void removeScheduledTripsWithoutLegs(boolean recreateManagedEntitiesIds, Mode... modes) {
    ...
    for(Mode mode : modes){
      ...
      var tripsWithEmptyLegTimings =
          servicesByMode.stream().filter( r -> r.getTripInfo().hasScheduleBasedTrips()).flatMap(
              r -> r.getTripInfo().getScheduleBasedTrips().stream().map(
                  rts -> Pair.of(r, rts))).filter(
              tripSchedulePair ->
                  !tripSchedulePair.second().hasRelativeLegTimings()).collect(Collectors.toList());
      if(!tripsWithEmptyLegTimings.isEmpty()){
        ...
        tripsWithEmptyLegTimings.forEach(
        ...
        for(var routedTripSchedule : routedService.getTripInfo().getScheduleBasedTrips()){
          ...
          for(var departure : routedTripSchedule.getDepartures().streamAscDepartureTime().collect(Collectors.toList())){
            if(prevDeparture != null && prevDeparture.getDepartureTime().equals(departure.getDepartureTime())){
              toBeRemovedDepartures =
                  (toBeRemovedDepartures==null) ? new ArrayList<>(2) : toBeRemovedDepartures;
   ...
   * {@inheritDoc}
   *
   * All managed ids of the layer are recreated via {@link #recreateRoutedServicesIds()},
   * {@link #recreateRoutedTripsIds()}, and {@link #recreateRoutedTripScheduleDepartureIds()}
  ...
  public void recreateRoutedTripScheduleDepartureIds() {
    /* allow modifier to recreate the scheduled trips departure entity ids. because these ids are unique across all
     modes, services, trips schedules for which each has a container we should only reset it once, otherwise it
      is no longer unique across those containers when recreating the ids */
  ...
  public void recreateRoutedTripsIds() {
    /* allow modifier to recreate the scheduled trips entity ids. because these ids are unique across all modes and
     services for which each has a container we should only reset it once, otherwise it is no longer unique
      across those containers when recreating the ids */
    boolean doIdReset = true;
    for(var routedModeServices : routedServicesLayer) {
      for (var entry : routedModeServices) {
        if(entry.getTripInfo().getScheduleBasedTrips().getFactory().getIdGroupingToken() !=
            entry.getTripInfo().getFrequencyBasedTrips().getFactory().getIdGroupingToken()){
          throw new PlanItRunTimeException("Expectation in updating ids is that all trips" +
              " (frequency and scheduled) use same id token, this was found to not be the case," +
              " please adjust implementation");
        }
  ...
  public void recreateRoutedServicesIds() {
    /* allow modifier to recreate the routed service layer managed entity ids. because routedServices' ids
    are unique across all modes for which each has a container we should only reset it once, otherwise it
     is no longer unique across those containers when recreating the ids */
    boolean doIdReset = true;
    for(var routedModeServices : routedServicesLayer){
