...
/**
 * Whenever routed services' trips managed Ids with an external id are changed in terms of their internal id, their
 * XML ids remain the same and might no longer be unique. When this is not desirable and the user wants to keep
 * the XML ids unique, for example when the network is persisted to disk afterwards, in which case the XML ids must be
 * unique, then this handler can be used to sync the XML ids to the newly assigned unique internal ids.
 *
...
public class SyncRoutedTripsXmlIdToIdHandler extends SyncXmlIdToIdHandler implements RoutedServicesModifierListener {
  ...
  public void onRoutedServicesModifierEvent(RoutedServicesModificationEvent event) {
    ...
    routedServicesLayer.forEach( rsm -> rsm.forEach( routedService ->
      {
        routedService.getTripInfo().getScheduleBasedTrips().forEach(this::syncXmlIdToInternalId);
        routedService.getTripInfo().getFrequencyBasedTrips().forEach(this::syncXmlIdToInternalId);
      }));
  }
