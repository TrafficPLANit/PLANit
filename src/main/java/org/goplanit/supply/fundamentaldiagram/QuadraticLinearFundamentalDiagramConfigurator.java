 ...
 * Note that a change on the link segment level takes precedence over a change on the link segment type, i.e., the most
 * specific overwrite is used in the final fundamental diagram applied on the link segment.
 * <p>
 ...
 * with the x-axis (density=0), by explicitly setting the capacity or moving the maximum density point the FD will
 * adjust its backward wave speed to accommodate any change compared to the default computed capacity/maximum density.
 * </p>
 * <p>
 *   For QL there is an option to force concavity (Default activated) even if the critical speed is not defined (in
 *   which case a percentage of free speed is applied (also configurable)). When critical speed is defined
 *   this speed is   assuming it is smaller than the free speed.
 * </p>
 * 
 * @author markr
 */
public class QuadraticLinearFundamentalDiagramConfigurator
    extends FundamentalDiagramConfigurator<QuadraticLinearFundamentalDiagramComponent> {
  ...
  /**
   * Set the capacity in pcu/h/lane to use for the Newell FD for a given link segment type. This only impacts
   * the backward wave speed used to keep the FD viable.
   *
