...
/**
 * Utilities to assist in prepping or filling low level array based costs for assignment or otherwise assuming
 * concrete cost instances and network (virtual or otherwise) are available.
 *
 * @author markr
 */
public class CostUtils {
  ...
  public static double[] createEmptyLinkSegmentCostArray(UntypedPhysicalNetwork<?,?> network, Zoning zoning){
    if(network.getTransportLayers().size()>1){
      //todo: eventually the costs should be tracked per layer as id numbering of link segments is only unique
      // per layer, for now we crash in case someone tries
      throw new PlanItRunTimeException("Link segment cost array can only be created if physical network only " +
          "has a single layer, multi-layer support has not yet been implemented");
    }
   ...
   */
  public static void populateModalVirtualLinkSegmentCosts(
      Mode mode, VirtualCost virtualCost, VirtualNetwork virtualNetwork, double[] costArray){
    if(virtualNetwork.getLayer().getConnectoidSegments().isEmpty()){
      LOGGER.warning("No connectoid segments found in provided virtual network, unable to populate " +
  ...
  public static void populateModalPhysicalLinkSegmentCosts(
    ...
    physicalCost.populateWithCost(
        (UntypedPhysicalLayer<?, ?, MacroscopicLinkSegment>) network.getLayerByMode(mode), mode, costArray);
  }
   ...
   * Note that this requires no edge segments of any other type than physical link segments to be present,
   * otherwise the indexing in the raw cost array may be inconsistent
   *
   * @param mode to use
   * @param physicalCost to apply to physical part of network
   * @param network physical network
   * @return generalised cost array by link segment id
   */
  public static double[] createAndPopulateModalSegmentCost(
      Mode mode, AbstractPhysicalCost physicalCost, UntypedPhysicalNetwork<?,?> network){
  ...
  /**
   * Populate entire cost array with both virtual and non-virtual link segment costs based on the concrete cost
   * classes, for given mode
   *
   * @param mode to use
   * @param virtualCost   to apply to virtual part of network
   * @param physicalCost to apply to physical part of network
  ...
  public static double[] createAndPopulateModalSegmentCost(
      ...
      UntypedPhysicalNetwork<?,?> network, Zoning zoning){
   ...
   * @param costArray to fill
   */
  public static void populateModalFreeFlowPhysicalLinkSegmentCosts(
      Mode mode, MacroscopicLinkSegments linkSegments, double[] costArray) {
