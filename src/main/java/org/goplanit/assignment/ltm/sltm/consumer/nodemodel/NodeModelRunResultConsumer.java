...
public interface NodeModelRunResultConsumer {
  ...
  /**
   * A centroid node is a special node where all flow is always accepted (destination incoming links), or sending flows
   * do not come from turns but from origin zone (origin exit links). As such we present only the original link
   * segment sending flows
   * 
   * @param node                   to use
   * @param linkSegmentSendingFlow to use. Note that these are the network wide sending flows by link segment id, not
   *                               localised for the node
   */
