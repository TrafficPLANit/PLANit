 ...
 * updated during this sending flow update because it only updates the sending flows of outgoing links of
 * potentially blocking nodes. When an incoming link of any node is not also an outgoing link of another
 * potentially blocking node its sending flow remains the same even if it actually changes due to further
 * upstream changes in restrictions. In this approach this is only identified when we make sure the sending
 * flows are updated during (this) loading on the path level. Hence, we must update sending flows here.
 * <p>
 * ANY OTHER SOLUTION APPROACH: Here we update all used nodes and sending flows are updated iteratively and
 * locally propagated without the need of the loading in the sending flow update. Therefore, there is no need to
 * update the sending flows. On the other hand we now update the turn flows on all used nodes rather than
 * only the potentially blocking ones.
 * 
 * @author markr
 *
 */
public class ConjugateBushTurnFlowUpdateConsumer
  ...
  /**
   * Track the turn accepted flows when they are classified as being tracked during network loading,
   * otherwise do nothing
   * 
   * @param conjEdgeSegment      the turn to use
  ...
  public TurnFlowAccessorConjugateSegments getAcceptedTurnFlows() {
    return TurnFlowAccessorConjugateSegments.of(this.compiledMovementIds, dataConfig.getAcceptedTurnFlows());
  }
}
