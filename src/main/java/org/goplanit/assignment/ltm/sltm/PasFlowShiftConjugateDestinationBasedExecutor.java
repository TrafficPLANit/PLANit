    ...
    while (index < pasAlternative.length-1) { // stop before merge
      ...
      if(totalBushAppliedFlowShift != 0){
        ...
        var networkResult = executeNetworkLevelTurnUpdate(
            theMode, assignmentStrategy, originalNetworkCosts, conjSegmentCosts, bushes,
...
//        pas.getStatus() != PasStatus.CONGESTED && pas.getStatus() != PasStatus.UNCONGESTED_POTENTIALLY_CONGESTED;
  ...
  protected Map<ConjugateDestinationBush, Double> executePasFlowShiftSplittingRatesOnly(
      ...
      Set<ConjugateDestinationBush> bushes,
      boolean logAll) {
    boolean updateAlphas = false;
    boolean updateSendingFlows = false;
  ...
  protected Map<ConjugateDestinationBush, Double> executePasFlowShift(
    ...
    var divergeFlowShiftResult = executeDivergeFlowShift(
        initialReferenceWeight, initialBushWeights, pasFlowShift, theMode,
        assignmentStrategy, originalNetworkCosts, conjSegmentCosts, bushes,
   ...
   * @param bushes to use
   * @param logAll               to use
   * @param smoothingApproach  to apply
   * @param additionalSmoothingFactor to factor in if regular smoothing is applied
   * @return total shifted flow for the reference turn used and indicator if we snapped to zero for the high cost segment
   */
  public Triple<EdgeSegment,Double,Boolean> performEquilibratedUncongestedFlowShifts(
      Mode theMode,
    ...
    if(this.pas.getStatus() == PasStatus.CONGESTED
      ...
      return Triple.of(pas.getFirstEdgeSegment(false),0.0, false);
    }
    ...
    do{
      ...
      var iterationS2FlowShiftPerBush = executePasFlowShift(
          ...
          conjSegmentCosts,
          bushes,
          updateNodeModel, //todo: for uncongested we can hopefully switch this off
          updateSendingFlows, updateOutFlows, updateCosts, // other flags
          logAll);
    ...
    if(totalPasShift > 0 && smoothingApproach != FlowShiftSmoothingApproach.OFF) {
      ...
      if(smoothingApproach == FlowShiftSmoothingApproach.NORMAL) {
        ...
        // reset to original situation by setting smoothing factor to 0
        smoothingFactor = 0;
      }
      ...
      var smoothedShiftResult = performFinalShiftForSmoothing(
          ...
          bushes,
          logAll,
          updateNodeModel, updateSendingFlows, updateOutFlows, updateCosts, // flags
          smoothingFactor,
          smoothingApproach);
  ...
  public void populateBushSubPathSendingFlows(
    ...
    for (var bush : pas.getRegisteredBushes()) {
      ...
      double remainingSubPathSendingFlow = conjBush.determineConstrainedSubPathSendingFlow(
          pas.getAlternative(lowCostSegment),
  ...
  public Triple<EdgeSegment,Double, Boolean> performEquilibratedCongestedFlowShifts(
    ...
    do{
      ...
      if (pas.pasId == 2945L) { // local PAS update
        int bla = 4;
        logAll = true;
...
//            theNode, consumer, networkLoading, networkLoading.getCurrentInflowsPcuH());
      ...
      //--------------- UPDATE SENDING FLOWS THROUGH ALTERNATIVE ------------------------------------
      bushS2RemainingSendingFlows.clear();
      populateBushSubPathSendingFlows(pas, false,
          networkLoading.getCurrentFlowAcceptanceFactors(), originalNlConsistentFlowAcceptanceFactors, bushS2RemainingSendingFlows);
  ...
  public Pair<EdgeSegment,Double> performOneShotFlowShiftWithNetworkSplittingRateUpdate(
    ...
    var conjStrategy = ((StaticLtmConjugateBushStrategy)assignmentStrategy);
    var networkLoading = conjStrategy.getLoading();
    if(pas.getFirstEdgeSegment(true) != referenceTurn){
      // flow shifts are performed from s1 perspective, so negate
      proposedFlowShift = -proposedFlowShift;
    }

    // expected to be part of mini-loading, so current alphas are always our reference point, so provide for both cases)s
    double s1SendingFlow = determinePasSubPathSendingFlow(pas, true,
        networkLoading.getCurrentFlowAcceptanceFactors(), networkLoading.getCurrentFlowAcceptanceFactors());
    ...
    //--------------- UPDATE SENDING FLOWS THROUGH ALTERNATIVE ------------------------------------
    bushS2RemainingSendingFlows.clear();
    populateBushSubPathSendingFlows(pas, false, networkLoading.getCurrentFlowAcceptanceFactors(),
        networkLoading.getCurrentFlowAcceptanceFactors(), bushS2RemainingSendingFlows); // again use current alphas
    ...
    if(s1ApproachingZero || s2ApproachingZero){
      // special case: when we have very little flow left we should shift it all
      // then avoid the assumed unnecessary smoothing delay to get to zero and allow full move to zero flow
      chosenFlowShift = s1ApproachingZero ? -s1SendingFlow : guaranteedS2SendingFlow;
      snapToZero = true;
    }
  ...
  //
  // due to aymmetric updates in case we hit zero flow, it can happen that some bushes will add flow over the course
  ...
  //
  // returned boolean indicates of we snapped to zero or not
...
//    double mostRestrictiveBushSmoothingFactor = Double.MAX_VALUE;
//    boolean finalS1S2SwappedComparedToStart = referenceAltInitialTurn.equals(pas.getFirstEdgeSegment(true));
//
...
//      if(stepSize < mostRestrictiveBushSmoothingFactor) {
//        mostRestrictiveBushSmoothingFactor = stepSize;
...
//      double proposedSmoothedTargetShiftedFlow = totalShiftedFlow * smoothingFactor;
...
//        //    so we now need to add to smooth, keep compensation positive as is
...
//        compensatoryS2ShiftToMeetTarget *= finalS1S2SwappedComparedToStart ? -1 : 1;
//
//        if (isDestinationTrackedForLogging() || logAll) {
...
//        totalRemovedFlow,
//        appliedFlowShiftsForInitialTurnByBush,
...
//      LOGGER.info(message);
//    }
//    return Pair.of(appliedShiftResult.values().stream().mapToDouble(e->e).sum(), false);
