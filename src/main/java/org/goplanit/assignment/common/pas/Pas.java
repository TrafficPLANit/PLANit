...
public class Pas<V extends DirectedVertex, ES extends EdgeSegment> implements Comparable<Pas<V, ES>> {
  ...
  public double computeGap(double s1Flow, double s2Flow){
    ...
    if(getAlternativeLowCost() == 0){
      // replace with halve the high cost cost as a proxy to avoid issues with division by zero
      return getReducedCost() * s2Flow
          /
          ( (getAlternativeHighCost() * 0.5) * (s1Flow + s2Flow));
    }

    return
        getReducedCost() * s2Flow
        /
        (getAlternativeLowCost() * (s1Flow + s2Flow));
  }
