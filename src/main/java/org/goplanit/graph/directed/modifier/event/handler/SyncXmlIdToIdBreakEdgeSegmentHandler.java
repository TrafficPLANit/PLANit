 ...
 * It is likely the user wants to keep the XML ids unique despite using
 * internal ids in the memory model. For example when the network is persisted to disk afterwards in which case
 * the XML ids can be used to map ids. In this situation the XML ids
 ...
 * sync all broken edge segments' xml id to the internal id of these
