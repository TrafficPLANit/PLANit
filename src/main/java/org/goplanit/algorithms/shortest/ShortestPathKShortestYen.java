 ...
 * possible (Yen, Jin Y. (Jul 1971). "Finding the k Shortest Loopless Paths in a Network".
 * Management Science. 17 (11): 712–716. doi:10.1287/mnsc.17.11.712)
 *<p>
 * See also <a href="https://en.wikipedia.org/wiki/Yen%27s_algorithm#cite_note-yenksp2-2">k-shortest path by Yen</a>
 * </p>
 * <p>
 * Yen's K-shortest path algorithm does not allow for cycles which makes it suitable for path choice. It is however
 * costly to run, especially without any additional constraints on these paths and using Dijkstra for all
 * intermediate path searches. However, it is easy to understand and a good benchmark implementation for other
...
public class ShortestPathKShortestYen implements ShortestPathOneToOne{
  ...
  /**
   * Find and mark all exit segments as excluded for all k-2-shortest paths that share the root path of the k-1
   * shortest path up until the provided forkNode index. These exit segments will be added to the to be provided
   * excludedLinksToPopulate.
   *
   * @param excludedLinksToPopulate add excluded link segments to this container
   * @param forkNodeIndex index of node up until it is expected that the rootpath coincides with the most recent
   *                      k-shortest path in kShortestRawPathsWithCost
   * @param kShortestRawPathsWithCost the so far found k-shortest paths of which the last entry is used as
   *                                  reference to identify rootpaths
   * @param excludedLinksThreshold break condition upon which we terminate search if we reach threshold
   *                               (inclusive, e.g., when container has x entries and threshold is x we stop)
   */
    ...
    for(int p = 0; p < numFoundKShortestPaths-1 ; ++p){
      ...
      if(coincidesUpToForkNode && pathPIterUpToFork.hasNext()){
        ...
        if(excludedLinksToPopulate.size() >= excludedLinksThreshold){
          // pre-emptive check in case we know we can break already (because all potential exist
          // segments are excluded for this fork)
          break;
   ...
   *
   * @param oneToAllBootStrapSearch per origin, use this once to construct one-to-all wide shortest paths most
   *                                efficiently
   ...
   * <p>
   *   find all fork node alternative shortest paths related to the k-1 shortest path in the set by starting at each
   *   node of the previous shortest path and excluding the adjacent links of that node from consideration when they
   *   have   been used in ANY preceding shortest path. Continue to find paths until all fork nodes are exhausted,
   *   after which   the single shortest alternative found is chosen as the next k-shortest path. It is added to
   *   the set and then we use that alternative to find the next shortest path forking off from that shortest path,
   *   repeat until k-shortest paths are found.
   * </p>
   *
  ...
  /**
   * Construct k-shortest paths from source node to sink based on directed LinkSegment edges but now with some
   * predefined banned segments.
   *
   * @param origin origin vertex of source node
   * @param destination destination vertex of sink node
  ...
  public KShortestPathResult executeOneToOne(
    ...
    /* only recompute bootstrap result if origin changes - one-to-all, so it is available for all destinations
     * at once */
    ...
    kShortestRawPathsWithCost.add(
            Pair.of(oneToAll.createRawPath(origin, destination), oneToAll.getCostToReach(destination)));
    DirectedVertex kMinus1ForkNode = origin;
    var excludedLinks = new TreeSet<EdgeSegment>();

    /* track potential candidates found when doing a pass along the most recent found k-shortest path by eliminating
       single links on that path from being used to find other paths which may or may not be the next shortest.

       we also keep track of the fork node of each potential path, i.e, location where it branched off from the
       previous shortest path to avoid unnecessary regeneration of already identified potential shortest paths
       during previous passes
     */
    var potentialShortestPathAlternatives =
            new TreeMap<Double, List<Pair<DirectedVertex, Deque<EdgeSegment>>>>(Comparator.naturalOrder());
    for(int k = 1; k < numShortestPaths; ++k){
      ...
      //todo: OPTIONAL by excluding only the single adjacent link and moving one node at the time the search space
      // becomes very large having minimumDetour filters can assist with this to speed up computation time and
      // exclude paths that are not considered eligible anyway
      ...
      for(int forkNodeIndex = 0; forkNodeIndex < kMinus1PathSize-1; ++forkNodeIndex){
        ...
        /* track cost up to the current chosen fork node, so we can reconstruct full alternative path cost once
         * we have it */
        ...
        /* skip any attempt to find alternative path using costly one-to-one path search until we reached the fork node
           ...
           added to the potential shortest paths because those links were part of k-2 shortest path which - when it
           was being constructed already identified all possible options by forking of that root path up to the fork
         */
        // todo: this could be made smarter as we now only track in relation to previous rootpath, but we could
        //  identify any coincidence with any rootpoth for found paths 0-(k-1) of course.
        ...
        var oneToOnePerOriginResult =
                oneToOnePerOriginSearch.executeOneToOne(
                        currForkSegment.getUpstreamVertex(), destination, excludedLinks);
        ...
        double potentialKShortestPathCost =
                currPotentialPathCostUpToFork + oneToOnePerOriginResult.getCostToReach(destination);
        ...
        potentialShortestPathAlternatives.get(potentialKShortestPathCost).add(
                Pair.of(currForkNode, potentialKShortestPath));
      }
      ...
      /* Select cheapest, remove it from candidates, track fork node used so we can skip some unneeded searches in
       * next round */
