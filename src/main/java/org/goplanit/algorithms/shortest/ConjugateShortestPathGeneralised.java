...
/**
 * Conjugate version of shortest path algorithm. The only difference is found in that the original network edge segment
 * costs are now obtained on the turn level, where each conjugate edge segment (turn) collects its cost by means of
 * its incoming original edge segment. Note that for the final turn this means the last edge segment's cost is missed
 * which is compensated for at the end. In its current form, it assumes a macroscopic network and macroscopic link
...
public class ConjugateShortestPathGeneralised extends ShortestPathGeneralised {
  ...
  /**
   * For a conjugate network we obtain the conjugate edge segment cost by means of the incoming original edge
   * segment of the turn, i.e., conjugate edge segment.
   * 
  ...
  protected Double getEdgeSegmentCost(final EdgeSegment edgeSegment) {
    ...
    return edgeSegmentCosts[
            (int) ((ConjugateEdgeSegment) edgeSegment).getOriginalAdjacentEdgeSegments().first().getId()];
  }
   ...
   */
  public ConjugateShortestPathGeneralised(
          final double[] originalEdgeSegmentCosts,
