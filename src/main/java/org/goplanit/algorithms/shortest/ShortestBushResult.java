...
/**
 * Interfaces that defines how to access results of a shortest bush execution allowing one to extract bushes or
 * cost information.
 * 
 * @author markr
 *
...
public interface ShortestBushResult extends ShortestResult {
   ...
   * origin-destination vertex. The direction of the result is inherited from the search itself, so if the search
   * was inverted, the dag has an inverted root etc.
   * 
   ...
   */
  public abstract ACyclicSubGraph createDirectedAcyclicSubGraph(
          final IdGroupingToken idToken, final DirectedVertex origin, final DirectedVertex destination);

  /**
   * Find the next edge segment for a given vertex, depending on the underlying search this can be either in
