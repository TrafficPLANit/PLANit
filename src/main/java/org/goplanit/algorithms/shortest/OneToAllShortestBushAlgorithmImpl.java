...
public class OneToAllShortestBushAlgorithmImpl extends OneToAllShortestGeneralisedAlgorithm implements OneToAllShortestBushAlgorithm {
  ...
  /**
   * Track incoming edge segment(s) that are shortest for each vertex in this array. In case there is only a single shortest option, the object is an edge segment, otherwise it
   * is a collection of edge segments
   */
  ...
  /**
   * Test whether shortest bush means less or equal cost compared to existing cost, so cheaper and equal cost paths are considered. also store the costs so
   * we can test if they were equal or shorter in {@link #processShorterOrEqualIncomginEdgeSegment(EdgeSegment)}
   */
  ...
  /** Whenever an incoming edge segment is considered shorter or equal, it should either be marked as the new incoming edge segment for the vertex (shorter), or supplement the existing
   * shortest segments present (equal). This is what this method does and it is supplied as consumer to the shortestAlgorithm base function
   * 
  ...
  private void processShorterOrEqualIncomginEdgeSegment(EdgeSegment incomingEdgeSegment) {
    ...
    if(currEqualShortestCosts) {    
      ...
      if(incomingEdgeSegmentsVertex  instanceof EdgeSegment) {
        ...
        incomingEdgeSegments[downstreamVertexId] = Arrays.asList(incomingEdgeSegment);
      }else{
  ...
  public ShortestBushResult executeOneToAll(DirectedVertex currentOrigin) throws PlanItException {
    ...
    /*
     * found shortest bush costs to each vertex for current origin. When deemed shortest, all incoming edge segments are stored on the array as a list, unless only a single edge segment is shortest
     * in which case the entry is just the edge segment
     */
