...
/**
 * A Populate physical cost event is fired when PLANit requests for a registered listener to populate the newly created
 * physical cost component. It is assumed only a single listener will populate this component and it is expected that
 * the registration of this listener is handled by the platform rather than the user. The end user will - via the
 * listener - receive this event when implementing an input builder and registering this builder on a PLANit project
 * for example.
 * 
 * @author markr
 *
...
public class PopulatePhysicalCostEvent extends PopulateUntypedComponentEvent {
  ...
  public static final PlanitComponentEventType EVENT_TYPE =
      new PlanitComponentEventType("PLANITCOMPONENT.COST.PHYSICAL.POPULATE");
   ...
   */
  public PopulatePhysicalCostEvent(
      final PlanitComponentFactory<?> source, AbstractPhysicalCost physicalCostToPopulate, MacroscopicNetwork network) {
